...
# -------------------------------
# Determine script_dir / REPO_PATH
# -------------------------------
try:
...
if not os.path.exists(podfile_path):
    print(f"❌ {FILE_TO_UPDATE} not found in path: {podfile_path}")
    exit(1)
...
if not current_version:
    print(f"❌ Could not find the dependency '{DEPENDENCY}' in {FILE_TO_UPDATE}.")
    exit(1)
...
if current_version != site_version:
    ...
    for line in lines:
        if pattern_version_only.search(line):
            new_line = pattern_version_only.sub(rf"\g<1>{site_version}\g<3>", line, count=1)
